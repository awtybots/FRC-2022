...
public class ProjectileMotionTest {
    ...
    private static void test() {
        ...
        Vector2 goalDisplacement =
                new Vector2(
                        3.0,
                        Field.kVisionTargetHeight
                                - (Camera.kMountingHeight + Camera.kShooterOffset.y));

        double launchVelStationary =
                projectileMotionSolver.getOptimalLaunchVelocityStationary(goalDisplacement);
        double launchRpmStationary =
                ShooterSubsystem.ballVelocityToFlywheelRpm(launchVelStationary);
            ...
            for (double i = -5.0; i <= 5.0; i += 1.0) {
                ...
                Vector2 launchData =
                        projectileMotionSolver.getOptimalLaunchVelocityMoving(
                                goalDisplacement, robotVelocity);
                if (launchData == null) {
